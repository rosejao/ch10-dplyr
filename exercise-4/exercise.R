...
flights <- mutate(time_gained = arr_delay - dep_delay) %>% 
  arrange(-time_gained)
...
summarise(to_sea, mean = mean(time_gained, na.rm = T))
...
summary <- flights %>% 
  ...
  summarize(
    ...
    min_time = mean(time_gained, na.rm = T)
    max_time = mean(time_gained, na.rm = T)
  )
